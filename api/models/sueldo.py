...
class Sueldo(models.Model):
    ...
    sueldo_monto_bono = models.DecimalField(max_digits=20, decimal_places=2) # = sueldo_bono_porcentaje * sueldo_valor_basico
    sueldo_mensual = models.DecimalField(max_digits=20, decimal_places=2) # = sueldo_valor_basico + sueldo_asignacion_familiar + sueldo_monto_bono
    sueldo_anual = models.DecimalField(max_digits=20, decimal_places=2) # = sueldo_mensual * 14
    activo = models.BooleanField(default=True)
    # ------------------------------------------------------------------
    def calcular_asignacion_familiar(self):
...
def validar_y_calcular_campos(sender, instance, **kwargs):
    ...
    rmv = DatosGlobales.objects.get(id=1, variable='RMV', anio=datetime.now().year).valor # ==== IMPORTANTE ==
    # Validar que sueldo_valor_basico sea mayor que la RMV
    if instance.sueldo_valor_basico <= rmv:
        raise ValueError("El sueldo básico debe ser mayor que la Remuneración Mínima Vital del año actual")

    instance.calcular_asignacion_familiar()
